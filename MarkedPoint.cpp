...
MarkedPoint::MarkedPoint(QString name, ccGenericPointCloud* cloud, unsigned index)
    : cc2DLabel(name)
    , mMarkedType(0)
    , mColor(255, 0, 0)
{
    cc2DLabel::addPoint(cloud, index);
}

void MarkedPoint::setColor(const QColor &color)
{
    mColor = color;
}

QColor MarkedPoint::getColor()
{
    return mColor;
}

void MarkedPoint::setMarkedType(ccHObject *markedType)
{
    mMarkedType = markedType;
}

ccHObject* MarkedPoint::getMarkedType()
{
    return mMarkedType;
}
...
inline void MarkedPoint::setSelected(bool state)
{
